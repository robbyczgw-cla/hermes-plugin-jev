# Jev for Hermes: technical reference

Start with the [README](../README.md) for the pinned preview install and safe defaults. This reference covers the hook contracts, full configuration, failure behavior, and test procedures.

## Compatibility

Requires Python 3.11+ and a Hermes checkout exposing `PluginContext.register_hook`, `register_middleware`, `get_config`, `iter_hook_callbacks`, and the hooks below. Tested against Hermes commits `4e06e5f38a` and `77ecc72bcdd5da0163cca21c8af0e95b26ba3426`; older releases may lack these contracts. No Hermes core changes are required. Use separate Python environments for different Hermes revisions; an editable install of one can leak newer submodules into tests against the other.
